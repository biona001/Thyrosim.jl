...
p = [0.00174155;     #S4
     ...
     5;              #KLAG
     1.3;            #k4dissolve
     0.12*0.88;           #k4excrete; originally 0.119 (change with dial 2)
     1.78;           #k3dissolve
     0.12*0.88;           #k3excrete; originally 0.118 (change with dial 4)
     3.2;            #Vp
     5.2]            #VTSH

function modify_params(p, dial, height=1.77, weight=70, sex=1, Vp_scale=False)
    # Modify T4/T3 secretion and absorption rates with dial.
    p[1] *= dial[1]
    p[44] *= dial[2]
    p[19] *= dial[3]
    p[46] *= dial[4]

    # Modify Vp with model.
    if Vp_scale
    	Vp = plasma_volume(height, weight, sex)
    	p[47] = Vp

        # Need to also modify VTSH?
    end 
    return p
end
