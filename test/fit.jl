...
#     T4_init_600, T3_init_600, TSH_init_600 = blakesley_my600_data[1, :]
    ...
    schneider_err = @sync @distributed (+) for i in 1:num_sample
        #calculate (y - μ)^2
        abs2(schneider_euthy_dose[i] - mean_passing_dose(current_iter, fitting_index, 
            schneider_height[i], schneider_weight[i], schneider_sex[i], schneider_tspan[i], 
            schneider_init_tsh[i], schneider_euthy_dose[i], schneider_init_dose[i]))
    end
    schneider_err /= 2 * abs2(p[64])
...
@everywhere function mean_passing_dose(
    ...
    return (maxdose + mindose) / 2
end
@everywhere function TSH_within_interval(sol, Vtsh)
    tot_loss = 0
...
function fit_all()
    # only fit S3, S4, B0, A0, hill function params, and variance params
    fitting_index = SharedArray{Int}(
        [30; 31;                  # Bzero, Azero
    ...
    return optimize(p -> objective(p, fitting_index, 
        ...
        NelderMead(), Optim.Options(time_limit = 82800.0, iterations = 20000, g_tol=1e-5))
end
